...
i32 wait_for_input(std::vector<EvalQueue> &pre_output_queues,
  ...
  for (i32 pu = 0; pu < pre_output_queues.size(); pu++) {
    // Since we didn't support gpu pipelining, prefer to fetch task from
    // different pipeline instance.
    // Assume number of gpus equals number of pipeline instances. 
    if (gpu_status.size() != 0 && gpu_status[pu]) {
      continue;
    }
...
void schedule(SchedulerArgs args) {
  ...
  while (true) {
    ...
    while (!has_work_to_do(buffer_queue, pipeline_status, gpu_status, work_togo)) {
      i32 pu = -1;
      std::tuple<std::deque<TaskStream>, EvalWorkEntry> input;
      if ((pu = wait_for_input(pre_output_queues, gpu_status, input)) == -1) {
        Intermediate finished;
...
grpc::Status WorkerImpl::NewJob(grpc::ServerContext* context,
  ...
  sArgs.num_gpu_threads = num_gpu_threads;
    ...
    for (i32 kg = 0; kg < num_kernel_groups; ++kg) {
      ...
      if (device_type == DeviceType::CPU) {
        ...
        for (i32 i = 0; i < max_devices; ++i) {
          ...
          assert(device_id < num_gpu_threads);
          for (size_t i = 0; i < group.size(); ++i) {
            KernelConfig& config = std::get<1>(group[i]);
            config.devices.clear();
            config.devices.push_back({device_type, device_id});
